...
class UsdMcpClient:
    ...
    # =============================================================================
    # Stage ID-based API Methods
    # =============================================================================

    async def open_stage(self, file_path: str, create_if_missing: bool = False):
        """Open an existing USD stage or create a new one, returning a stage_id
        ...
        Returns:
            Stage ID or None on error
        """
        if not self.session:
            raise ValueError("Not connected to server")

        print(f"\n--- Opening stage at {file_path} ---")
        result = await self.session.call_tool("open_stage", {
            "file_path": file_path,
        ...
        try:
            ...
            if response.get("ok", False):
                ...
                print(f"Success: Stage opened with ID {stage_id}")
                return stage_id
            else:
                logger.error(f"Error: {response.get('message', 'Unknown error')}")
                return None
    ...
    async def close_stage_by_id(self, stage_id: str, save_if_modified: bool = True):
        """Close a USD stage by its ID
        ...
        print(f"\n--- Closing stage with ID {stage_id} ---")
        result = await self.session.call_tool("close_stage_by_id", {
            "stage_id": stage_id,
    ...
    async def analyze_stage_by_id(self, stage_id: str, include_attributes: bool = False):
        ...
        print(f"\n--- Analyzing stage with ID {stage_id} ---")
        result = await self.session.call_tool("analyze_stage_by_id", {
            "stage_id": stage_id,
    ...
    async def create_mesh_by_id(self, stage_id: str, prim_path: str, points: List[tuple], 
                                face_vertex_counts: List[int], face_vertex_indices: List[int]):
        ...
        Returns:
            Mesh information or None on error
        """
        if not self.session:
            raise ValueError("Not connected to server")

        print(f"\n--- Creating a mesh at {prim_path} ---")
        result = await self.session.call_tool("create_mesh_by_id", {
            "stage_id": stage_id,
            "prim_path": prim_path,
            "points": points,
            "face_vertex_counts": face_vertex_counts,
    ...
    async def create_primitive_by_id(self, stage_id: str, prim_type: str, prim_path: str, 
                                     size: float = 1.0, position: tuple = (0, 0, 0)):
        """Create a geometric primitive in a USD stage
        ...
        print(f"\n--- Creating {prim_type} at {prim_path} ---")
        result = await self.session.call_tool("create_primitive_by_id", {
            "stage_id": stage_id,
            "prim_type": prim_type,
            "prim_path": prim_path,
            "size": size,
    ...
    async def create_reference_by_id(self, stage_id: str, prim_path: str, 
                                     reference_file_path: str, reference_prim_path: str = ""):
        """Add a reference to an external USD file
        ...
        print(f"\n--- Adding reference to {reference_file_path} at {prim_path} ---")
        result = await self.session.call_tool("create_reference_by_id", {
            "stage_id": stage_id,
            "prim_path": prim_path,
            "reference_file_path": reference_file_path,
            "reference_prim_path": reference_prim_path
    ...
    async def create_material_by_id(self, stage_id: str, material_path: str, 
                                    diffuse_color=(0.8, 0.8, 0.8), metallic=0.0, roughness=0.4):
        """Create an OmniPBR material in a USD stage
        ...
        print(f"\n--- Creating material at {material_path} ---")
        result = await self.session.call_tool("create_material_by_id", {
            "stage_id": stage_id,
            "material_path": material_path,
            "diffuse_color": diffuse_color,
            "metallic": metallic,
    ...
    async def bind_material_by_id(self, stage_id: str, prim_path: str, material_path: str):
        """Bind a material to a prim in the USD stage
        ...
        print(f"\n--- Binding material {material_path} to {prim_path} ---")
        result = await self.session.call_tool("bind_material_by_id", {
            "stage_id": stage_id,
            "prim_path": prim_path,
            "material_path": material_path
        })
    ...
    async def set_transform_by_id(self, stage_id: str, prim_path: str, 
                                  translate=None, rotate=None, scale=None, time_code=None):
        """Set transform for a prim using stage ID

        Args:
            stage_id: ID of the stage
            prim_path: Path to the prim to transform
            translate: Optional XYZ translation values
            rotate: Optional XYZ rotation angles in degrees
            scale: Optional XYZ scale values
        ...
        print(f"\n--- Setting transform for {prim_path} ---")
        result = await self.session.call_tool("set_transform_by_id", data)
    ...
    async def visualize_scene_graph_by_id(self, stage_id: str, output_format: str = "text", 
                                          output_path: Optional[str] = None, max_depth: int = -1,
                                          include_properties: bool = False):
        ...
        Args:
            ...
            output_format: Format of visualization ('text', 'html', 'json', 'network')
            output_path: Path to save the output file (required for non-text formats)
            max_depth: Maximum depth to display (-1 for unlimited)
            include_properties: Whether to include prim properties

        Returns:
            Visualization information or None on error
        """
        if not self.session:
            raise ValueError("Not connected to server")
        ...
        print(f"\n--- Visualizing scene graph in {output_format} format ---")
        result = await self.session.call_tool("visualize_scene_graph_by_id", data)
        ...
        try:
            ...
            if response.get("ok", False):
                ...
                if output_format == "text" and "visualization" in viz_data:
                    print("\nScene Graph:")
                    print(viz_data["visualization"])
                elif output_path and "output_path" in viz_data:
                    print(f"Visualization saved to: {viz_data['output_path']}")
                return viz_data
            else:
                logger.error(f"Error: {response.get('message', 'Unknown error')}")
                return None
